...
// @version      1.5.4
// @description  检查洛谷题解格式，辅助通过审核 + AI 聊天公式渲染
...
(async function () {//contributor: DeepSeek,Copilot Free,Sunny_boybgfcxc (luogu uid 1144516)
    'use strict';

    // 通用 JS 库加载器（下载 + 缓存 + 执行）
    function loadLibrary(url, cacheKey) {
        let cached = GM_getValue(cacheKey);
        if (cached) {
            try {
                let fn = new Function(cached);
                fn();
                console.log(`[库] ${cacheKey} 从缓存加载成功`);
                return Promise.resolve();
            } catch (e) {
                console.warn(`[库] ${cacheKey} 缓存执行失败，重新下载`, e);
    ...
    // CSS 加载器（下载 + 缓存 + 注入，并修正字体路径）
    function loadCSS(url, cacheKey) {
        let cached = GM_getValue(cacheKey);
        if (cached) {
            let fixed = cached.replace(/url\(fonts\//g, 'url(https://cdnjs.cloudflare.com/ajax/libs/katex/0.16.9/fonts/');
            GM_addStyle(fixed);
            return Promise.resolve();
        }
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onload: function (res) {
                    if (res.status >= 200 && res.status < 300) {
                        let css = res.responseText;
                        GM_setValue(cacheKey, css);
                        // 字体路径替换为 jsdelivr 的地址
                        let fixed = css.replace(/url\(fonts\//g, 'url(https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/fonts/');
                        GM_addStyle(fixed);
                        resolve();
                    } else {
                        reject(new Error(`HTTP ${res.status}`));
...
`.trim();
    ...
    async function initLibraries() {
        try {
            await loadLibrary('https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/katex.min.js', 'katex_cache');
            await loadLibrary('https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/contrib/auto-render.min.js', 'auto_render_cache');
            await loadLibrary('https://cdn.jsdelivr.net/npm/marked@9.1.6/marked.min.js', 'marked_cache');
            await loadLibrary('https://cdn.jsdelivr.net/npm/dompurify@3.0.6/dist/purify.min.js', 'purify_cache');
            await loadCSS('https://cdn.jsdelivr.net/npm/katex@0.16.9/dist/katex.min.css', 'katex_css_cache');
            console.log('[库] 所有资源加载完成');
        } catch (e) {
            console.error('[库] 加载失败:', e);
        }
    }
    ...
    function renderAiChatBubbleContent(bubble, content) {
        ...
        var katex = unsafeWindow.katex || window.katex;
        var marked = unsafeWindow.marked || window.marked;
        var DOMPurify = unsafeWindow.DOMPurify || window.DOMPurify;

        if (!marked || !DOMPurify || !katex || typeof katex.renderToString !== 'function') {
            bubble.textContent = content;
            return;
        }
        ...
        try {
            ...
            var walker = document.createTreeWalker(
                ...
                NodeFilter.SHOW_TEXT,
                {
                    acceptNode: function (node) {
                        return node.nodeValue && /\$/.test(node.nodeValue) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_REJECT;
                    }
                }
            );
            var textNodes = [];
            ...
            for (var i = 0; i < textNodes.length; i++) {
                ...
                // 行间公式 $$...$$
                var processed = text.replace(/\$\$([\s\S]+?)\$\$/g, function (match, formula) {
                    var clean = formula.replace(/^[\r\n]+|[\r\n]+$/g, '');
                    try {
